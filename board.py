...
class Board:
    ...
    def apply_move(self, move, color):
        ...
        """
        #print(move in self.get_valid_moves(color))
        ...
        for i in range(8):
            for j in range(8):
                ...
                if self.board[i][j] == EMPTY:
                    empty += 1
        if empty == 0 or (self.get_valid_moves(BLACK) == [] and self.get_valid_moves(WHITE) == []):
            if white > black:
                if color == BLACK:
                    return LOSE
                if color == WHITE:
